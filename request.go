...
func NewClientRequestMessage(conn io.Reader) (*ClientRequestMessage, error) {
	...
	case TypeIPv4:
		...
		message.Address = net.IP(buf).String()
	case TypeDomain:
...
func WriteRequestSuccessMessage(conn io.Writer, ip net.IP, port uint16) error {
	...
	_, err := conn.Write([]byte{Version, ReplySuccess, ReservedField, addressType})
	if err != nil {
		return err
	}
	_, err = conn.Write(ip)
...
func WriteRequestFailureMessage(conn io.Writer, replyType ReplyType) error {
	_, err := conn.Write([]byte{Version, replyType, ReservedField, TypeIPv4, 0, 0, 0, 0, 0})
	return err
}
